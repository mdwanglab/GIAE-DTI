...
class GNNEncoder(nn.Module):
    ...
    def get_embedding(self, x, edge_index, mode="cat", l2_normalize=False):
        ...
        self.eval()
        assert mode in {"cat", "last"}, mode  # "last" only gets the last layer of embedding
        x = self.create_input_feat(x)
        edge_index = to_sparse_tensor(edge_index, x.size(0))
        out = []
        for i, conv in enumerate(self.convs[:-1]):
...
class GAE(nn.Module):
    ...
    def train_epoch(
        ...
        global edge_loss, degree_loss, z
        ...
        for perm in DataLoader(
            ...
            if self.degree_decoder is not None and alpha:
                ...
                loss2 = alpha * F.mse_loss(self.degree_decoder(z).squeeze(), deg)
            ...
            edge_loss += loss1.item()
            degree_loss += loss2.item()
